## Connect from the extension UI (0.2.6)

Open **AirCodum Webview → Connection**. Under **Server settings & troubleshooting → Connection address → Change**, choose **Local Wi-Fi / Ethernet** for devices on the same network, or **Tailscale** for remote access. The selection starts the listener on that address. Use **QR code → Show QR code**, or **Enter manually** to copy the displayed host, port and pairing key. Local Wi-Fi requires mobile 2.4.2; Tailscale QR works with 2.4.1 or later.

Local Wi-Fi does not require Tailscale. Traffic is unencrypted, so use a network you trust. Allow Local Network access on iOS. If connection fails, check that the phone uses the computer’s private IP, both devices share the same network, and the firewall/router permits connections on port 11040. Guest-network client isolation can prevent access.
...
2. Set application-level `aircodum.bindAddress` to the desktop's private LAN or actual Tailscale IP, or keep localhost behind a TLS reverse proxy. Servers reject wildcard/public binds. An address in the Tailscale range alone does not create encryption.
3. Run **Start AirCodum Server**, then **AirCodum: Copy Pairing Token** in the Command Palette.
4. In the original mobile app, enter the host, port **11040**, and pairing token. Select TLS for a certificate-validated HTTPS proxy, **Local Wi-Fi** for the same-network connection, or **Tailscale** for encrypted remote access. A TLS proxy must forward WebSocket upgrades and Authorization headers.
...
QR is optional; manual LAN/TLS/Tailscale host, port and token entry remains supported. QR images are generated locally and hidden when the listener changes. Both mobile platforms validate the payload, store the credential in SecureStore and use the existing authenticated handshake.
